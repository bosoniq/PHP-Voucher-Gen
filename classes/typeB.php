...
/**
* Create type B vouchers with a particular arrangement of submitted data 
* presented as a hex value
*/
class typeB extends newVoucher
{
	
	/**
	 * Create type B vouchers with a particular arrangement of submitted data presented 
	 * as a hex value
	 * 
	...
	public function createVouchers()
	{
		...
		$string = $this->offer_ref.'%d'.$this->date.$this->employee;
	...
	/**
	 * Receives a voucher code which it deconstructs to the original data used to generate 
	 * the code
	 * 
	...
	public function returnData($original_String) 
	{
		...
		$basic_values['employee'] = substr($original_String, -3, 3);
		$basic_values['date'] = substr($original_String, -9, 6);
